import { ActionRowBuilder, ButtonBuilder, ButtonInteraction, CacheType, Colors, EmbedBuilder, RoleSelectMenuBuilder, StringSelectMenuBuilder, StringSelectMenuInteraction } from "discord.js";
...
const handleBosyuu = async (i: ButtonInteraction<CacheType>) => {
  ...
  const members = embed.toJSON().description?.split("\n").slice(1).map((m) => m.match(/<@!?(\d+)>/)?.[1]).filter((m) => m) as string[] | undefined;
  if (!members) return;
  ...
  if (i.customId === "bosyuu_leave") {
    ...
    await i.update({
      embeds: [
        embed.setDescription(
          `参加者一覧\n${members.filter((m) => m !== i.user.id).map((m) => `・<@${m}>`).join("\n")}`
        ),
      ],
    });
  }
  if (i.customId === "bosyuu_cancel") {
    if(i.user.id  !== embed.toJSON().footer?.text) {
      await i.reply({ content: "あなたはこの募集を作成したユーザーではありません。", ephemeral: true });
      return;
    }
    await i.update({
...
const handleVerify = async (i: ButtonInteraction<CacheType>) => {
  ...
  if (!hasRole) {
    await i.reply({ content: "認証用の役職が設定されていません。", ephemeral: true });
    return;
  }
  if (Array.isArray(i.member.roles) ? i.member.roles.includes(verifyRole) : i.member.roles.cache.has(verifyRole)) {
    await i.reply({ content: "あなたはすでに認証されています。", ephemeral: true });
    return;
  }
  if(Array.isArray(i.member.roles)) {
    await i.reply({ content: "認証ができない状態のようです...もう一度お試しください", ephemeral: true });
    return;
  }
    ...
    components: [
      new ActionRowBuilder<StringSelectMenuBuilder>()
        .addComponents(
          ...
          .addOptions(...role.menu.options.map((o) => ({
            label: o.name,
            value: o.id,
            emoji: o.emoji,
...
const handleRolePanelRole = async (i: StringSelectMenuInteraction<CacheType>) => {
  ...
  const current = i.member.roles.cache.filter((r) => role.menu.options.map((o) => o.id).includes(r.id)).map((r) => r.id);
